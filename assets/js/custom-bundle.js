...
(function ($) {
    ...
    function setHeadingPadding($off, $el = $(document)) {
        ...
        if (head.length)
            head.css('--dsn-head-height', (head.get(0).offsetHeight) + "px");
    }
    ...
    function sliderPortfolio($el = $(document)) {
        ...
        const initSlider = async function () {
            ...
            if (!isPadding || window.innerWidth <= 575)
                return false;

            const bgSlider = this.querySelector(".bg-container")?.cloneNode(true);
